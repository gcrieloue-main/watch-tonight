...
function App() {
  ...
  useEffect(() => {
    ...
    loadMovies("/api/" + (urls[category] || urls["default"]));
  }, [page, category]);

  useEffect(() => {
    if (category === CATEGORY_WATCHED) {
      console.log("load watched movies");
      const url = "/api/watched?ids=" + JSON.stringify(watchedIds);
      loadMovies(url);
    }
  }, [category, watchedIds]);
  ...
  function addWatchdId(id) {
    ...
    fetch(`http://localhost:3001/api/watch/${id}`).then(async () => {
  ...
  function removeWatchdId(id) {
    ...
    fetch(`http://localhost:3001/api/watch/delete/${id}`).then(async () => {
